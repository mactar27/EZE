    ...
    >
      <div className="container mx-auto px-6 md:px-12 lg:px-24 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          <ul className="flex items-center gap-6">
            {NAV_LINKS.map((link) => (
              <li key={link.name}>
                <Link
                  href={link.href}
                  className="text-dark/80 hover:text-primary font-medium text-base transition-colors"
                >
                  {link.name}
                </Link>
              </li>
          ...
          <Link
            href="#contact"
            className="bg-primary text-white px-6 py-2.5 rounded-full font-medium text-base hover:bg-primary/90 transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
          >
            Demander un devis
          </Link>
        </nav>
